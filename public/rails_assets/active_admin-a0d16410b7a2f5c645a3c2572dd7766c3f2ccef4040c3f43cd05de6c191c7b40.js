...
(function() {
  $(function() {
    var hide_credentials_form, hide_new_admin_form, show_credentials_form, show_new_admin_form;
    $('.new_admin #admin_user_password_confirmation').keypress(function(e) {
      if (e.which === 13) {
        e.preventDefault();
        return $("#admins .create_link").click();
      }
    });
    $('tr.admin #admin_user_password_confirmation').keypress(function(e) {
      if (e.which === 13) {
        e.preventDefault();
        return $(this).closest("tr.admin").find(".update_link").click();
      }
    });
    hide_credentials_form = function(tr) {
      tr.find(".form").hide();
      tr.find(".admin_actions").show();
      return tr.find(".form_actions").hide();
    };
    show_credentials_form = function(tr) {
      tr.find(".form").show();
      tr.find(".admin_actions").hide();
      return tr.find(".form_actions").show();
    };
    $('document').on('click', "#admins .admin .credentials_link", function(e) {
      var btn, tr;
      e.preventDefault();
      btn = $(this);
      tr = $(btn.attr("data-id"));
      return show_credentials_form(tr);
    });
    $('document').on('click', "#admins .admin .cancel_link", function(e) {
      var btn, tr;
      e.preventDefault();
      btn = $(this);
      tr = $(btn.attr("data-id"));
      return hide_credentials_form(tr);
    });
    $('document').on('click', "#admins .admin .update_link", function(e) {
      var btn, form, tr;
      e.preventDefault();
      btn = $(this);
      tr = $(btn.attr("data-id"));
    ...
    show_new_admin_form = function() {
      ...
      return $("#admin_user_email").focus();
    };
    hide_new_admin_form = function() {
      $("#admins .form.new_admin").hide();
      $("#admins .new_admin_actions .form_actions").hide();
      return $("#admins .new_admin_actions .new_link").show();
    };
    $("#admins .new_link").click(function(e) {
      e.preventDefault();
      return show_new_admin_form();
    });
    $(".new_admin_actions .cancel_link").click(function(e) {
      e.preventDefault();
      return hide_new_admin_form();
    });
    return $("#admins .create_link").click(function(e) {
      var form;
      e.preventDefault();
...
(function() {
  $(function() {
    ...
    return setTimeout(hideFlashes, 3000);
  });

}).call(this);
(function() {
  $(function() {
    var hide_update_btn, show_saved_state, show_update_btn;
    show_update_btn = function(el) {
      return $(el).closest('tr').find('.update_link').show();
    };
    hide_update_btn = function(el) {
      return $(el).closest('tr').find('.update_link').hide();
    };
    $('#settings .form form input, textarea').change(function() {
      return show_update_btn(this);
    });
    $('#settings .form form input, textarea').keypress(function() {
      return show_update_btn(this);
    });
    $('#settings .form form').submit(function() {
      return false;
    });
    show_saved_state = function(el) {
      var hide_label, saved_label;
      saved_label = el.closest('tr').find('.saved-state');
      el.hide();
      saved_label.show();
      hide_label = function(label, btn) {
        return label.fadeOut((function(_this) {
          return function() {
            return btn.show();
          };
        })(this));
      ...
      return setTimeout((function() {
        return hide_label(saved_label, el);
      }), 1000);
    };
    return $('#settings .update_link').click(function(e) {
      var btn, form, tr;
      e.preventDefault();
      btn = $(this);
      tr = $(btn.attr("data-id"));
...
(function() {
  ...
  enable_redactor = function() {
    if (!window.redactor_settings) {
      window.redactor_settings = {};
    }
    $('#settings_tabs').bind('easytabs:after', function(event, $clicked, $targetPanel, settings) {
      var text_input;
      text_input = $targetPanel.find('.settings-redactor');
      if (text_input.css("display") !== "none") {
        return text_input.redactor(window.redactor_settings);
      }
    });
    return $('#settings_tabs .settings-redactor:visible').redactor(window.redactor_settings);
  };
